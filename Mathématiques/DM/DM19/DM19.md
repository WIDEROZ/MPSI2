...
Alors, 
$$\begin{array}{rl}
m_{r+1} - m &= \frac{3}{N+1}- \frac{ 3N_{r}+\frac{5}{2}N_{v} +\frac{7}{2}N_{b}}{N(N+1)} \\
 &= \frac{3}{N(N+1)} \times \left( N-\left( N_{r}+\frac{5}{6}N_{v} +\frac{7}{6}N_{b}\right) \right) \\
&=\frac{3}{N(N+1)} \times \left( N_{v} + N_{b} - \frac{5}{6}N_{v} - \frac{7}{6}N_{b} \right)
\end{array}$$
Donc, 
...
- <u>Si</u> $N_{v} > N_{b}$, <u>l'espérance va donc augmenter si on ajoute une boule rouge.</u> 
- <u>Si</u> $N_{v} < N_{b}$ <u>l'espérance diminuera. </u>
- <u>Si</u> $N_{v} = N_{b}$ <u>elle restera la même. </u>


##### On ajoute un dé vert dans l'urne
Par analogie au raisonnement précédent on a :
$$m_{v+1}-m = \frac{1}{2N(N+1)} \times - \left(2N_{b} + N_{r} \right)$$
Ainsi, 
...
Par analogie au raisonnement précédent on a :
$$m_{b+1}-m = \frac{1}{2N(N+1)} \times \left(  N_{r} + 2N_{v}\right)$$

Comme $N, N_{r}$ et $N_{v}$ sont positifs, 
$m_{b+1}-m_{r} \geq 0$ 

Ainsi, 
- <u>Si</u> $N_{v} = N_{r} = 0$, <u>l'espérance reste la même</u>
- <u>Sinon, l'espérance augmente</u>

___
Soit $k \in \mathbb{N}^{*}$, 
Pour $\alpha \in \{ r, v, b \}$ on note $m_{\alpha + k}$ l'espérance de $X$ lorsqu'on ajoute $k$ dés de couleur $\alpha$. 
Comme :
$$m_{r+k} = \frac{3N_{r}+\frac{5}{2}N_{v}+ \frac{7}{2}N_{b}}{N+k} + \frac{3k}{N+k}$$
et que $3N_{r}+\frac{5}{2}N_{v}+ \frac{7}{2}N_{b}$ et $N$ sont des constantes
On a : 
$$\lim_{ k \to + \infty } m_{r+k} = \frac{3k}{N+k} = \frac{3}{\frac{N}{k}+1}= 3$$
Par analogie : 
$$\lim_{ k \to + \infty } m_{v + k} = \frac{\frac{5}{2}}{\frac{N}{k}+1} = \frac{5}{2} $$
et
$$\lim_{ k \to + \infty } m_{b + k} = \frac{\frac{7}{2}}{\frac{N}{k}+1} = \frac{7}{2}$$

Ainsi, on a :
$$\boxed{\forall \alpha \in \{ r, v, b \}, \lim_{ k \to +\infty } m_{\alpha + k} =   \sum_{i = 1}^{4}iP(X_{\alpha} = i) z= E(X_{\alpha})}$$
___
##### On Prends le cas ou $N_{v} < N_{b}$
![[Pasted image 20240512150148.png]]
On a bien pour $x \in \mathbb{N}$, 
$$\begin{cases}
m \geq m_{r+x} \\
m \geq m_{v+x} \\
m \leq m_{b+x}
...
##### On Prends le cas ou $N_{v} > N_{b}$
![[Pasted image 20240512150419.png]]
On a bien pour $x \in \mathbb{N}$, 
$$\begin{cases}
m \leq m_{r+x} \\
m \geq m_{v+x} \\
...
#### Question 7
Comme les tirages sont physiquement indépendants car on tire avec remise, $R, V$ et $B$ sont des variables aléatoires mutuellement indépendantes. 
Mais comme :
$$P(R = 4, V = 5, B = 6) = P((R = 4) \cap (V = 5) \cap (B = 6))$$
et que $R, V$ et $B$ sont mutuellement indépendantes :
$$P(R = 4, V = 5, B = 6)  = P(R =4)P(V = 5)P(B = 6)$$
or 
- $P(R = 4) = \frac{4}{15} = \frac{N_{a}}{N} = p_{r}$
- $P(V = 5) = \frac{5}{15} = \frac{N_{v}}{N} = p_{v}$
- $P(B = 6) = \frac{6}{15} = \frac{N_{b}}{N} =  p_{b}$

Ainsi, 
$$\boxed{P(R = 4, V = 5, B = 6) = p_{r}p_{v}p_{b}}$$

#### Question 8
Soit $n \in N^{*}$
On note, les événements : 
$$\begin{cases}
R_{n} : \text{"} \text{Tirer un dé rouge au }n^{\text{eme}} \text{ tirage}\text{"} \\
V_{n} : \text{"} \text{Tirer un dé vert au }n^{\text{eme}} \text{ tirage}\text{"} \\
B_{n} : \text{"} \text{Tirer un dé bleu au }n^{\text{eme}} \text{ tirage}\text{"}
\end{cases}$$
(respectivement verts, bleus) tirés au $n^{\text{ieme}}$ tirage. 
Alors, comme
$R_{n}, V_{n}$ et $B_{n}$ sont disjoints et de réunion contraire $(R_{n}, V_{n}, B_{n})$ est un système complet d'événement

On peut appliquer la formule des probabilités totales : 
$$\begin{array}{rl}
P(R_{n+1}) &= P(R_{n})P(R_{n+1} | R_{n}) \\
& \space + P(V_{n})P(R_{n+1} | V_{n})  \\
&\space + P(B_{n})P(R_{n+1} | B_{n}) \\
\end{array}$$
$$\begin{array}{rl}
P(V_{n+1}) &= P(R_{n})P(V_{n+1} | R_{n}) \\
& \space + P(V_{n})P(V_{n+1} | V_{n})  \\
&\space + P(B_{n})P(V_{n+1} | B_{n})
\end{array}$$
$$\begin{array}{rl}
P(B_{n+1}) &= P(R_{n})P(B_{n+1} | R_{n}) \\
& \space + P(V_{n})P(B_{n+1} | V_{n})  \\
&\space + P(B_{n})P(B_{n+1} | B_{n})
\end{array}$$

Comme on enlève une boule à tirage, au $n$ eme tirage on a $N-(n-1)$ boules restantes dans l'urne. 

$$P((R = 4) \cap (V = 5) \cap (B = 6))$$
$$= P(R = 4)P((V = 5)|(R = 4))P((B = 6) | (R = 4) \cap (V = 5))$$
$$= \prod_{i = 0}^{3}\frac{N_{a}-i}{N-i} $$