...
Soit $n \in \mathbb{N}^{*}$, 
Par l'absurde supposons que $S_{1}$ converge : 
Il existe un $M \in \mathbb{R}$, tel que
$$\sum_{\sqrt{ k } \in [\![1, n]\!]} \frac{1}{k} \leq M$$
Comme : 
$$\sum_{k = 1}^{n} \frac{1}{k} \geq \sum_{\sqrt{ k } \in [\![1, n]\!]} \frac{1}{k}$$
$$\sum_{\sqrt{ k } \in [\![1, n]\!]} \frac{1}{k} \leq M \leq \sum_{k \in \mathbb{N}} \frac{1}{k}$$
Car $S$ diverge. 
