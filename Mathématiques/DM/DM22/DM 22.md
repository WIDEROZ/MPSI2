...
___
$$\sum_{(p, q) \in \mathbb{N}^{2}} \frac{1}{p^{2} + q^{2}}$$

$$I_{k} = \{ (p, q) \in (\mathbb{N}^{*})^{2} | p^{2} + q^{2} = k \}$$
Alors
$$\sum_{k = 1}^{+ \infty} \sum_{(p, q) \in I_{k}} \frac{1}{k} = \sum_{k = 1}^{ + \infty} \frac{1}{k} \left| I_{k}\right| = \sup_{F \in \mathcal{F}(\mathbb{N}^{*})}\left( \sum_{k \in F} \frac{1}{k} \left| I_{k}\right| \right)$$

Si $I_{k}$ est fini alors comme :
$$\sum_{k \in \mathbb{N}^{*}} \frac{1}{k} \text{ DV}$$
et que $\left| I_{k}\right| \geq 0$, 